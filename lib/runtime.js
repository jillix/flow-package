...
function saveRequire (handler) {
    try {
        const msg_handler = handler;
        handler = require(handler);
        if (typeof handler !== 'function') {
            return Promise.reject(new Error('Flow-registry.runtime.fn: Handler "' + msg_handler + '" is not a function.'));
        }
        return handler;
    } catch (err) {
        Promise.reject(err);
...
exports.fn = (fnIri, role, callback) => {
    return getFnDetails(fnIri).then((details)=> {
        ...
        if (isExcluded(details.owner + '/' + details.module)) {
            return details.fnPath;
        }

        return dependency.install(details).then(() => {
            return details.fnPath;
        });
    }).then(saveRequire);
};
